...
export default {
  name: "SupTl_TicketSettingRequester",
  props:['teamID'],
  data() {
    return {
      dialog: false,
  ...
  methods: {
    addRequester() {
      ...
      this.requester.teamID = this.teamID
      this.$store
        .dispatch("addRequester", this.requester)
        .then(() => {
          setTimeout(() => {           
            requester_promise.push(
              SparrowService.getRequesterByTeam(this.teamID)
              .then(response => {
                  requester = response.data
              })
            )
            Promise.all(requester_promise).then(()=>{
              this.$store.dispatch('fetchRequesterByTeam',this.teamID)
              this.$store.state.requesters_by_team[0] = requester
              this.dialog = false;
              this.toUpdate = false;
              this.toAdd = true;
    ...
    updateRequester() {
      ...
      SparrowService.putRequester(this.requester_id, params).then(()=>{
          setTimeout(() => {
            requester_promise.push(
              SparrowService.getRequesterByTeam(this.teamID).then(response => {
                requester = response.data;
              })
            )
            Promise.all(requester_promise).then(()=>{
