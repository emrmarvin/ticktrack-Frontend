<template>
  <v-card width="480" class="pa-5 mx-10">
    ...
    <v-list-item
      v-for="business_unit in $store.state.business_units_by_team[0]"
...
export default {
  name: "SupTL_TicketSettingsBusinessUnit",
  props:['teamID'],
  data() {
    return {
      dialog: false,
  ...
  methods: {
    addBusinessUnit() {
      ...
      this.$store
        ...
        .then(() => {
          setTimeout(() => {           
            bu_promise.push(
              SparrowService.getBusinessUnitByTeam(this.teamID)
              .then(response => {
                  bu = response.data 
              })
            )
            Promise.all(bu_promise).then(()=>{
              this.$store.dispatch('fetchBusinessUnitsByTeam',this.teamID)
              this.$store.state.business_units_by_team[0] = bu
              this.dialog = false;
              this.toUpdate = false;
              this.toAdd = true;
    ...
    updateBusinessUnit() {
      ...
      SparrowService.putBusinessUnit(this.bu_id, params).then(()=>{
          setTimeout(() => {
            bu_promise.push(
              SparrowService.getBusinessUnitByTeam(this.teamID).then(response => {
                bu = response.data
              })
            )
            Promise.all(bu_promise).then(()=>{
