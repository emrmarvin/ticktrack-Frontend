...
export default {
  name: "SupTL_TicketSettingsSubCat",
  props:['sub_categories','teamID'],
  data() {
    return {
      dialog: false,
      dialogEdit: false,
  ...
  methods: {
    ...
    addTicketSub1Category() {
      if(this.validateMainCategory() == true){
        ...
        this.sub1_Category.teamID = this.teamID
        this.$store
          .dispatch("addTicketSub1Category", this.sub1_Category)
          .then(() => {
            setTimeout(() => {
              sub_cat_promise.push(
                SparrowService.getTicketSub1_CategoryByTeam(this.teamID)
                .then((response) => {
              ...
              Promise.all(sub_cat_promise).then(()=>{
                this.sub1_Categories_By_Team = sub_categories;
    ...
    updateSubCategory(){
      ...
      SparrowService.putTicketSub1Cat(this.subcat_id, params).then(()=>{
          setTimeout(() => {
            sub_cat_promise.push(
              SparrowService.getTicketSub1_CategoryByTeam(this.teamID).then(response => {
                sub_categories = response.data;
              })
            )
            Promise.all(sub_cat_promise).then(()=>{
                this.$store.state.sub1_Categories_By_Team[0] = sub_categories;
                this.sub1_Categories = sub_categories;
                this.dialog = false;
                this.toUpdate = false;
                this.toAdd = true;
