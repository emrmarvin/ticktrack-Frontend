<template>
  <v-card width="480" class="pa-5 mx-10">
    ...
    <v-list-item
      v-for="status in $store.state.ticket_Status_by_team[0]"
      :key="status.id"
      three-line
    >
      <v-list-item-content>
...
export default {
  name: "SupTL_TicketSettingsStatus",
  props:['teamID'],
  data() {
    return {
      dialog: false,
  ...
  methods: {
    addTicketStatus() {
      let status = []
      let status_promise = []
      this.status.teamID = this.teamID
      this.$store
        .dispatch("addTicketStatus", this.status)
        .then(() => {
          setTimeout(() => {
            status_promise.push(
              SparrowService. getTicketTicket_Status_By_Team(this.teamID)
              .then(response => {
            ...
            Promise.all(status_promise).then(()=>{
              ...
              this.$store.state.ticket_Status_by_team[0] = status
    ...
    updateStatus(){
      ...
      SparrowService.putTicketTicket_Status(this.status_id, params).then(()=>{
          setTimeout(() => {
            status_promise.push(
              SparrowService.getTicketTicket_Status_By_Team(this.teamID).then(response => {
                status = response.data;                
              })
            )
            Promise.all(status_promise).then(()=>{
