          ...
          <v-container>
              ...
              >
                ...
                <template v-slot:item="{ item, attrs, on }">
                  <v-list-item v-on="on" v-bind="attrs" #default="{ active }" @click="columnsToDisplay(item.text,item.value)">
                    <v-list-item-action>
                    <v-checkbox :disabled="true" :id="item.value" :input-value="active"></v-checkbox>
                    </v-list-item-action>
                    <v-list-item-content>
                      <v-list-item-title>
        ...
        >
         ...
         <template v-slot:[`item.world_area`]="{ item }">
          ...
          <template v-slot:[`item.ticket_Status.status`]="{ item }">
            <v-chip v-bind:style="statusColor(item.ticket_Status.status)">{{item.ticket_Status.status}}</v-chip>
         </template>

         <template v-slot:[`item.service_rating`]="{ item }">
            <v-chip v-bind:style="ratingColor(item.service_rating)">{{item.service_rating}}</v-chip>
         </template>

         <template v-slot:[`item.classification.label`]="{ item }">
            <v-chip v-bind:style="classificationColor(item.classification.label)">{{item.classification.label}}</v-chip>
         </template>
          ...
          <template v-slot:[`item.actions`]="{ item }">
            ...
            <v-icon v-if="show_delete==true" small @click="delete_ticket(item.id, item.ticket_Status.status)"> mdi-delete </v-icon>
          </template>
        </v-data-table>
      </v-col>
    </v-row>
  ...
  methods: { 
    dynamicExportHeaders(){
      ...
      for(let i=0;i < this.headers.length; i++){
        if(this.headers[i].text !== "Actions" ){
          this.$store.state.dynamicExportReport[this.headers[i].text] = this.headers[i].value
        }
      } 
    },
    statusColor(status){
    ...
    columnsToDisplay(text,value){
      ...
      if(checkBox.checked == false){      
        this.headers.splice(this.headers.length-1,0,{text:text, value: value})
        this.dynamicExportHeaders()
