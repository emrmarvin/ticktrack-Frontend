...
methods:{
  ...
  startWatch() {
      ...
      this.$store.state.runningTickets[this.running_ticket_index].save_Ticket = setInterval(this.updateTicket, 900000)
  },
  stopWatch() {
  ...
  updateTicket(){
      let ticket_param ={
        ...
        total_hrs:0,
        running_time:this.$store.state.runningTickets[this.running_ticket_index].time.substr(0,5),
      ...
      SparrowService.putTicket(this.ticket.id,ticket_param).then(()=>{
       })
    },
