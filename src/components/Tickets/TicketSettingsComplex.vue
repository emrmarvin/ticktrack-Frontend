...
export default {
  ...
  created() {
    this.complexities = this.$store.state.complexitiesByTeam[0]
  },
  computed:{
    getTeams(){
  ...
  methods: {
    ...
    addComplexity() {
      if(this.validateMainCategory() == true){
        ...
        this.complexity.teamID = this.$store.state.teamID
        this.$store
    ...
    updateComplexity(){
      ...
      SparrowService.putTicketComplexity(this.complexity_id, params).then(()=>{
          setTimeout(() => {
            SparrowService.getTicketComplexitiesbyTeam(this.$store.state.teamID).then(response => {
              this.complexities = response.data;
              this.$store.state.complexitiesByTeam[0] = response.data;
              this.dialog = false;
              this.toUpdate = false;
              this.toAdd = true;
