<template>
  <v-card width="480" class="pa-5 mx-10">
    ...
    <v-list-item
      v-for="main_Category in mainCategories"
  ...
  methods: {
    ...
    addTicketMainCategory() {
      if(this.validateMainCategory() == true){
        ...
        this.main_Category.teamID = this.$store.state.teamID
        this.$store
          ...
          .then(() => {
            setTimeout(() => {
              ...
              SparrowService.getTicketMain_CategoryByTeam(this.$store.state.teamID)
                .then(response => {
