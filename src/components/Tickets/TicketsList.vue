<template>
  <div>
    ...
    >
      <v-icon
        style="background-color: white; border-radius: 50px; margin-right: 5px"
        color="primary"
        >mdi-plus</v-icon
      >
      Add New Ticket
    ...
    >
      ...
      <v-icon color="white"> mdi-delete </v-icon> Delete Selected Tickets
    </v-btn>
    ...
    <v-tooltip top>
      ...
      <span class="text-center"
        >This is just to show your tickets from the previous FY, <br />The
        current FY that is set in the tool will not be change.</span
      >
    </v-tooltip>
    ...
    <v-row>
      ...
      <v-col cols="10">
        ...
        <v-select
          v-if="
            $store.state.filterValue.toLowerCase() == 'completed' ||
            $store.state.filterValue.toLowerCase() == 'closed' ||
            $store.state.filterValue.toLowerCase() == 'done'
          "
        ...
        >
          <template v-slot:selection="{ item, index }">
            <span style="margin-right: 5px" v-if="index === 0">{{
              item.label
            }}</span>
          ...
          <template v-slot:activator="{ on, attrs }">
            ...
            >
              <v-icon
                ...
                >mdi-filter</v-icon
              >
              Filter:{{ $store.state.filterValue }}
            </v-btn>
          </template>
          <v-list>
            <v-list-item-group
        ...
        <router-link to="/import"
          ><v-btn
            :disabled="disable_add_button"
            color="#FFA700"
            style="height: 54px; float: right; margin-right: 5px; color: white"
            ><v-icon
              style="
                background-color: white;
                border-radius: 50px;
    ...
    <v-row v-show="isCardView">
      ...
      <v-col
        style="padding-left: 45%; padding-top: 50px"
        v-show="$store.state.no_ticket_data"
      >
        No data to display
      </v-col>
      ...
      <v-col
        cols="12"
        md="6"
        v-for="(ticket, index) in this.$store.state.userTickets[0]"
        :key="ticket.id"
      >
        <div>
          <v-card
            elevation="15"
            class="pa-4 ticket_card ticket_card-main"
            v-if="index < limit_by"
            @click="
              ($event) =>
                selectcard($event, ticket.id, ticket.ticket_Status.status)
            "
          >
            <v-card-title
              class="title mb-3"
              v-bind:style="statusColor(ticket.ticket_Status.status)"
            >
              {{ ticket.title }}
              <v-row align="center" justify="end">
                <v-btn
                ...
                <v-btn
                  ...
                  small
                  @click="
                    getTicketToDelete(ticket.id, ticket.ticket_Status.status)
                  "
                >
            ...
            <v-card-subtitle
              v-show="ticket.period == null"
              class="ticket-description mb-4 text_color--text"
            >
              <b>Request Category : </b> {{ ticket.main_Category.label }} <br />
              <b>Request Type :</b>
              <span v-if="ticket.sub1_Category != null"
                >{{ ticket.sub1_Category.label }} </span
              ><br />
              <b>Classification : </b> {{ ticket.classification.label }} <br />
              <b>Due Date :</b> {{ formatDisplayDate(ticket.actual_due_date) }}
              <br />
              <b>Service Level Agreement :</b>
              {{ ticket.service_Level_Agreement }}(hrs)<br />
            </v-card-subtitle>
            <!------->

            <!--Ticket Info to Display if Completed-->
            <v-card-subtitle
              v-if="ticket.period != null"
              class="ticket-description mb-4 text_color--text"
            >
              <b>Request Category : </b> {{ ticket.main_Category.label }} <br />
              <b>Classification : </b> {{ ticket.classification.label }} <br />
              <b>Period :</b> {{ ticket.period.label }}<br />
              <b>Service Rating :</b> {{ ticket.service_rating.toFixed(2)
              }}<br />
              <b>Service Level Agreement :</b>
              {{ ticket.service_Level_Agreement }}(hrs)<br />
              <b>Actual Processing Time :</b> {{ ticket.total_hrs }}<br />
              <span v-if="$store.state.showTimer"
                ><b>Actual Time Spent :</b> {{ ticket.running_time }}</span
              >
            </v-card-subtitle>
            <!------->
      ...
      <div
        v-if="$store.state.totalTicketCount > default_limit"
        style="width: 100%; text-align: center"
      >
        <v-btn
          @click="ShowMore(default_limit, $store.state.totalTicketCount)"
          >{{
            limit_by >= $store.state.totalTicketCount
              ? "Hide more"
              : "Show more"
          }}</v-btn
        >
          ...
          <template v-slot:activator="{ on, attrs }">
            ...
            >
              <v-icon
                ...
                >mdi-filter</v-icon
              >
              Filter: <span v-if="nf_filterValue == true">On-Going</span>
    ...
    <v-row v-if="isCardView">
      ...
      <v-col
        cols="4"
        md="4"
        v-for="nf in $store.state.non_functional_tickets[0]"
        :key="nf.id"
      >
        <v-card elevation="15" class="pa-4 ticket_card">
          <v-card-title
            ...
            v-bind:style="
              nf.status == true
                ? 'background-color:rgb(255, 167, 0);color:white!important'
          ...
          <v-card-subtitle class="ticket-description mb-4 text_color--text">
            ...
            <b>Time Spent:</b> {{ formatTotalHours(nf.total_hrs) }} (HH:mm)
            <br />
            <span v-if="nf.period_ID != null"
              ><b>Period:</b> {{ nf.period.label }}</span
            >
          </v-card-subtitle>
      ...
      <v-card>
        ...
        <v-card-text v-if="isLoaded">
          <v-container>
            <v-row>
                  ...
                  >
                    <template v-slot:activator="{ on, attrs }">
                      <v-text-field
                        ...
                        v-on="on"
                      >
                        <v-icon slot="prepend" class="required"> * </v-icon>
                      </v-text-field>
                    </template>
              ...
              <v-col cols="12" md="12">
                <v-text-field v-model="ticket_Info.title" label="Title">
                  <v-icon slot="prepend" class="required"> * </v-icon>
                </v-text-field>
              </v-col>
              <v-col cols="12" md="12">
              ...
              <v-col
                cols="12"
                sm="12"
                v-if="this.ticket_Info.classification_ID == 1"
              >
                <v-select
              ...
              <v-col cols="12" sm="6">
                ...
                <v-select
                  ...
                  v-on:change="
                    getSub1CategoriesByMainCategory(
                      ticket_Info.main_Category_ID
                    )
                  "
                  item-value="id"
          ...
          <v-icon
            slot="prepend"
            style="margin-right: 5px; font-size: 30px !important"
            class="required"
          >
            *
          </v-icon>
          <b>Indicates that the field is required.</b> <br />
          <v-chip
            class="ma-2"
            label
            color="red"
            text-color="white"
            v-for="errors in formErrors"
            :key="errors"
          >
        ...
        <v-container v-if="!isLoaded" style="height: 400px">
          <v-row class="fill-height" align-content="center" justify="center">
            <v-col class="subtitle-1 text-center" cols="12"> KALMA ... </v-col>
            <v-col cols="6">
              <v-progress-linear
    ...
    <v-dialog persistent v-model="delete_dialog" width="480">
      <v-card>
        <v-card-title
          ...
          style="color: white; background-color: #004b8d"
        >
          Delete
        </v-card-title>
        <v-card-text>
          <div
            ...
            class="mb-5 mt-5 text_color--text"
          >
          ...
          <div class="justify-center" style="padding-left: 150px">
            <v-btn
              ...
              >YES</v-btn
            >
            <v-btn text @click="delete_dialog = false" style="color: #004b8d"
              >NO</v-btn
            >
          </div>
        </v-card-text>
    ...
    <v-dialog v-model="nf_dialog" persistent max-width="800px">
      <v-card>
        ...
        <v-card-text v-if="isLoaded">
          <v-container>
            <v-row>
              <v-row>
                <v-col cols="11" sm="6">
                  ...
                  >
                    ...
                    <v-date-picker v-model="nf_start_date" no-title scrollable>
                      <v-spacer></v-spacer>
              ...
              <v-col cols="12" md="12">
                <v-text-field v-model="non_functional.title" label="Title">
                  <v-icon slot="prepend" class="required"> * </v-icon>
                </v-text-field>
              </v-col>
              <v-col cols="12" md="12">
              ...
              <v-col cols="12" sm="12" v-if="non_functional.status == false">
                <v-row>
                  ...
                  <v-col cols="11" sm="6">
                    <input
                      v-model="nf_date_completed_time"
                      class="time-text"
                      type="time"
                      id="meeting-time"
                      name="meeting-time"
          ...
          <v-chip
            ...
            color="red"
            text-color="white"
            v-for="errors in non_functional_formErrors"
            :key="errors"
          >
            <v-icon left> mdi-alert </v-icon>
            {{ errors }}
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="close_nf()"> Close </v-btn>
          <v-btn
            v-if="nf_toSave"
            color="blue darken-1"
            text
            @click="addNF_Activity"
          >
            Save
          </v-btn>
          <v-btn
    ...
    <v-dialog persistent v-model="delete_dialog_nf" width="480">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-btn
            type="submit"
            @click="deleteNonFunctional"
            style="color: white"
            color="#004b8d"
            >YES</v-btn
          >
          <v-btn text @click="delete_dialog_nf = false" style="color: #004b8d"
            >NO</v-btn
          >
        </v-card-text>
      </v-card>
  ...
  data() {
    return {
      ...
      non_functional_status: [
        { id: 1, label: "On-Going", value: true },
        { id: 2, label: "Done", value: false },
      ],
      isLoaded: true,
      loader_ticket: true,
      loader_nf: true,
    ...
    setTimeout(() => {
      if (this.$store.state.userTickets[0] != null) {
        ...
        for (let i = 0; i < this.$store.state.userTickets[0].length; i++) {
          this.$store.state.userTickets[0][i].actual_due_date =
            this.formatDisplayDate(
              this.$store.state.userTickets[0][i].actual_due_date
            );
          this.$store.state.userTickets[0][i].start_Date =
            this.formatDisplayDate(
              this.$store.state.userTickets[0][i].start_Date
            );
        }
  ...
  methods: {
    ...
    bulkdelete(view, selected) {
      if (view == "Card") {
        ...
        for (let i = 0; i < tickets.length; i++) {
          ...
          this.ticketToDelete[1] =
            this.selectedtickets[tickets[i]].toLowerCase();
          this.deleteTicket();
        }
        ...
        for (let i = 0; i < selected.length; i++) {
          ...
          this.ticketToDelete[1] =
            selected[i].ticket_Status.status.toLowerCase();
          this.deleteTicket();
        }
      }
    },
    ...
    showTicketByFY(fy_label) {
      ...
      get_ticket_promise.push(
        this.$store.dispatch({
          type: "fetchUserTickets",
          UserId: this.$store.state.userID,
          FY: fy_label,
        }),
        this.$store.dispatch({
          type: "fetchNonFunctionalActivities",
    ...
    addTicket() {
      ...
      if (this.validateTicketInfoData() == true) {
        ...
        (this.ticket_Info.running_time = "00:00"),
          (this.ticket_Info.start_Date =
            this.formatStartDate + "T" + this.start_date_time + ":00");
        if (this.ticket_Info.classification_ID == "2") {
          //Project
          this.ticket_Info.actual_due_date =
            this.formatActualDueDate + "T" + this.actual_due_date_time + ":00";
          this.ticket_Info.service_Level_Agreement =
            Computations.ProjectSLAComputation(
              this.formatStartDate + "T" + this.start_date_time + ":00",
              this.formatActualDueDate + "T" + this.actual_due_date_time + ":00"
            );
        } else {
          ...
          for (
            let i = 0;
            i < this.$store.state.complexitiesByTeam[0].length;
            i++
          ) {
            if (
              this.$store.state.complexitiesByTeam[0][i].id ==
              this.ticket_Info.complexity_ID
            ) {
              this.ticket_Info.service_Level_Agreement =
                this.$store.state.complexitiesByTeam[0][i].no_of_hrs;
              if (this.ticket_Info.service_Level_Agreement >= 9) {
                number_of_days_to_work =
                  this.ticket_Info.service_Level_Agreement / 9;
                let hours_to_add = this.ticket_Info.service_Level_Agreement % 9;
                initial_sla = moment(this.ticket_Info.start_Date)
                  .add(number_of_days_to_work, "days")
                  .format("YYYY-MM-DDTHH:mm:ss");

                if (hours_to_add != 0) {
                  initial_sla = moment(initial_sla)
                ...
                let time_remaining_in_shift =
                  this.$store.state.user_shift.end_Time -
                  converted_time.toFixed(2);
                if (
                  time_remaining_in_shift <
                  this.ticket_Info.service_Level_Agreement
                ) {
                  initial_sla = moment(this.ticket_Info.start_Date)
                    .add(15 + this.ticket_Info.service_Level_Agreement, "hours")
                    .format("YYYY-MM-DDTHH:mm:ss");
                } else {
                  initial_sla = moment(this.ticket_Info.start_Date)
                    .add(this.ticket_Info.service_Level_Agreement, "hours")
                    .format("YYYY-MM-DDTHH:mm:ss");
                }
              }
        ...
        this.$store
          ...
          .then(() => {
            ...
            setTimeout(() => {
              add_promise.push(
                SparrowService.getTicketsByUser(
                  this.$store.state.userID,
                  this.$store.state.activeFY.label
                ).then((response) => {
                  this.dialog_saving = false;
                  ...
                  for (let i = 0; i < response.data.length; i++) {
                    if (
                      response.data[i].ticket_Status.status.toLowerCase() ==
                      this.$store.state.inProgressStatus.toLowerCase()
                    ) {
            ...
            Promise.all(add_promise).then(() => {
              ...
              this.$store.state.filterValue =
                this.$store.state.inProgressStatus;
              this.disabledExportButton = true;
            });
          })
          .catch((err) => {
    ...
    getTicketToUpdate(id) {
      ...
      let ticket_promise = [];
      ticket_promise.push(
        SparrowService.getTicket(id).then((response) => {
          ...
          this.ticket_Info.service_Level_Agreement =
            response.data.service_Level_Agreement;
          this.ticket_Info.adjusted_Service_Level_Agreement =
            response.data.adjusted_Service_Level_Agreement;
          ...
          this.actual_due_date_time = response.data.actual_due_date.substr(
            11,
            19
          );
          ...
          if (response.data.date_Completion != null) {
            ...
            this.date_completed_time = response.data.date_Completion.substr(
              11,
              19
            );
          } else {
      ...
      SparrowService.putTicket(this.ticketID, ticket_param).then(() => {
        ...
        setTimeout(() => {
          SparrowService.getTicketsByUser(
            ...
            this.$store.state.allTickets = response.data;
            for (let i = 0; i < response.data.length; i++) {
              response.data[i].actual_due_date = this.formatDisplayDate(
                response.data[i].actual_due_date
              );
              response.data[i].start_Date = this.formatDisplayDate(
              ...
              if (this.$store.state.filterValue != "") {
                ...
                if (
                  ...
                  response.data[i].ticket_Status.status.toLowerCase() ==
                  this.$store.state.completedStatus.toLowerCase()
                ) {
    ...
    updateTicket() {
      if (this.validateTicketInfoData() == true) {
        ...
        let ticket_param = {
          ...
          actual_due_date:
            this.formatActualDueDate + "T" + this.actual_due_date_time,
          ...
          adjusted_Service_Level_Agreement:
            this.ticket_Info.adjusted_Service_Level_Agreement,
        ...
        //Project
        if (this.ticket_Info.classification_ID == 2) {
          ticket_param.service_Level_Agreement =
            Computations.ProjectSLAComputation(
              this.formatStartDate + "T" + this.start_date_time,
              this.formatActualDueDate + "T" + this.actual_due_date_time
            );
        } else {
          //Transactional
          ...
          let adjusted_sla = 0;
          if (
            this.ticket_Info.adjusted_Service_Level_Agreement != null &&
            this.ticket_Info.adjusted_Service_Level_Agreement >
              ticket_param.service_Level_Agreement
          ) {
            adjusted_sla =
          ...
          for (
            ...
            if (
              ...
              if (sla >= 9) {
                ...
                initial_sla = moment(ticket_param.start_Date)
                  .add(number_of_days_to_work, "days")
                  .format("YYYY-MM-DDTHH:mm:ss");
              ...
              if (adjusted_sla != 0) {
                let end_shift = 0;
                let initial_due_date = moment(initial_sla)
                  .add(adjusted_sla, "hours")
                  .format("YYYY-MM-DDTHH:mm:ss");

                if ((this.$store.state.user_shift.end_Time = 24)) {
                  end_shift = 0;
                } else {
                  end_shift = this.$store.state.user_shift.end_Time;
                }

                if (
                  moment(initial_due_date).format("dddd") == "Saturday" ||
                  moment(initial_due_date).format("dddd") == "Sunday"
                ) {
                  initial_due_date = moment(initial_due_date)
                    .add(2, "days")
                    .format("YYYY-MM-DDTHH:mm:ss");
                } else if (
                  moment(initial_due_date).format("dddd") == "Sunday"
                ...
                if (
                  parseInt(moment(initial_due_date).format("HH")) <
                    this.$store.state.user_shift.start_Time &&
        ...
        if (this.isCompleted == true) {
          ...
          let date_completion =
            this.formatDateCompleted + "T" + this.date_completed_time;
          ticket_param.period_ID = SparrowService.pickPeriod(
            this.$store.state.periods[0],
            date_completion
          );
          setTimeout(() => {
            if (this.ticket_Info.classification_ID == 2) {
              ...
              this.actual_time_spent = Computations.TimeSpentCalculation(
                this.formatStartDate + "T" + this.start_date_time,
                this.formatDateCompleted + "T" + this.date_completed_time
              );
              ticket_param.total_hrs = parseFloat(
                this.actual_time_spent
              ).toFixed(2);
              ticket_param.date_Completion =
                this.formatDateCompleted + "T" + this.date_completed_time;
              ticket_param.service_rating =
                Computations.serviceRatingComputation(
                  parseFloat(this.actual_time_spent).toFixed(2),
                  SLA
                ).toFixed(2);
    ...
    validateTicketInfoData() {
      ...
      if (
        ...
        if (this.ticket_Info.classification_ID == "1") {
          if (this.ticket_Info.complexity_ID) {
            if (this.isCompleted) {
              if (
                this.ticket_Info.quality_Rating_ID &&
                this.formatDateCompleted &&
                this.date_completed_time
              ) {
                if (
      ...
      if (this.isCompleted) {
        ...
        if (
          ...
          moment(this.formatStartDate + "T" + this.start_date_time).format(
            "YYYY-MM-DDTHH:mm:ss"
          )
        ) {
          this.formErrors.push(
            "Date completion is less than date it was started."
          );
        }
    ...
    validateNonFunctionalData() {
      ...
      if (
        this.non_functional.title &&
        this.formatNFStartDate &&
        this.nf_start_date_time &&
        this.non_functional.non_functional_classification_ID
      ) {
    ...
    addNF_Activity() {
      ...
      if (this.validateNonFunctionalData() == true) {
        let start_date =
          this.formatNFStartDate + "T" + this.nf_start_date_time + ":00";
        this.non_functional.status = true;
        this.non_functional.ticket_Owner_ID = this.$store.state.userID;
        this.non_functional.start_Date =
          this.formatNFStartDate + "T" + this.nf_start_date_time;
        ...
        setTimeout(() => {
          this.$store
            .dispatch("addNonFunctionalActivities", this.non_functional)
            .then(() => {
              setTimeout(() => {
                SparrowService.getNonFunctionalTickets(
                  this.$store.state.userID,
                  this.$store.state.activeFY.label
                ).then((response) => {
                  let activeNonFunctional = [];
                  ...
                  this.$store.state.non_functional_tickets[0] =
                    activeNonFunctional;
              ...
              this.$store.dispatch({
                type: "fetchNonFunctionalActivities",
                id: this.$store.state.userID,
                fy: this.$store.state.activeFY.label,
              });
              this.nf_filterValue = true;
              this.nf_filter_period = [];
              this.nf_start_date = "";
    ...
    getNF_Activity(id) {
      ...
      nf_promise.push(
        SparrowService.getNonFunctionalTicket(id).then((response) => {
          ...
          if (response.data.date_Completion != null) {
            this.nf_date_completed = response.data.date_Completion.substr(
              0,
              10
            );
            this.nf_date_completed_time = response.data.date_Completion.substr(
              11,
              19
            );
          } else {
    ...
    updateNF_Activity() {
      if (this.validateNonFunctionalData() == true) {
        ...
        let nf_param = {
          ...
          non_Functional_Classification_ID:
            this.non_functional.non_functional_classification_ID,
        };

        if (this.non_functional.status == false) {
          nf_param.period_ID = SparrowService.pickPeriod(
            this.$store.state.periods[0],
            this.formatNFStartDate
          );

          let start_date =
            this.formatNFStartDate_TimeSpent + " " + this.nf_start_date_time;
          let completion_date =
            this.formatDateCompleted_TimeSpent +
            " " +
            this.nf_date_completed_time;
          let timespent = moment.utc(
            moment(completion_date, "DD/MM/YYYY hh:mm:ss").diff(
              moment(start_date, "DD/MM/YYYY hh:mm:ss")
            )
          );
          let timespent_duration = moment.duration(timespent);
          let timespent_formatted = moment()
            .hour(timespent_duration._data.hours)
            .minute(timespent_duration._data.minutes)
            .format("HH:mm");
          var ts_hoursMinutes = timespent_formatted.split(/[.:]/);
          ts_hours = parseInt(ts_hoursMinutes[0], 10);
          ts_minutes = ts_hoursMinutes[1]
            ? parseInt(ts_hoursMinutes[1], 10)
            : 0;
          var actual_time_spent = ts_hours + ts_minutes / 60;
          nf_param.date_Completion =
            this.formatNFDateCompleted + "T" + this.nf_date_completed_time;
          nf_param.total_hrs = parseFloat(actual_time_spent).toFixed(2);
          this.updateInPorgressTickets(
            ts_hours,
            ts_minutes,
            parseFloat(actual_time_spent).toFixed(2)
          );
          this.$store.state.ongoingNonFunctional -= 1;
        }
    ...
    updateInPorgressTickets(addHours, addMinutes, nf_time_spent) {
      ...
      let activetickets = [];
      this.dialog_saving_nfa = true;
      this.dialog_saving_nfa_text =
        "Adjusting the Due date of your on-ging tickets";
      for (let i = 0; i < this.$store.state.activeTickets.length; i++) {
        get_ticket_promise.push(
          SparrowService.getTicket(this.$store.state.activeTickets[i].id).then(
            (response) => {
              initial_due_date = moment(response.data.actual_due_date)
                .add(addHours, "hours")
              ...
              ticket_param.push({
                ...
                adjusted_Service_Level_Agreement:
                  parseFloat(response.data.service_Level_Agreement) +
                  parseFloat(nf_time_spent),
      ...
      Promise.all(get_ticket_promise).then(() => {
        for (var i = 0; i < ticket_param.length; i++) {
          SparrowService.putTicket(ticket_param[i].id, ticket_param[i]).then(
            (response) => {
    ...
    deleteNonFunctional() {
      ...
      let activeNonFunctional = [];
      this.$store.state.ongoingNonFunctional = 0;
      SparrowService.deleteNonFunctionalTicketsA(
        this.non_functional_to_delete
      ).then(() => {
        setTimeout(() => {
          promise.push(
            SparrowService.getNonFunctionalTickets(
